#!/usr/bin/env python3
"""Run RNN offline ablations and summarize eval_IMUHOI metrics."""
import argparse
import json
import math
import re
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run eval_IMUHOI.py offline ablations for the RNN pipeline.")
    ...
    root = _repo_root()
    output_dir = Path(args.output_dir) if args.output_dir else root / "outputs" / f"rnn_offline_ablation_{time.strftime('%m%d%H%M')}"
