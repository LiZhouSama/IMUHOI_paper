...
from train.diffussion.loss.velocity_contact_loss import VelocityContactLoss
...
class VelocityContactTrainer(BaseTrainer):
    ...
    def model_forward(
        ...
        """训练直接用GT，推理时才调用上游HP的推理。"""
        hp_out = None
        gt_arg = batch if use_gt_targets else None

        if force_inference or (not use_gt_targets):
            if self.hp_model is not None:
                with torch.no_grad():
                    if hasattr(self.hp_model, "inference"):
                        hp_out = self.hp_model.inference(data_dict, sample_steps=sample_steps)
                    else:
                        print("Failed to get hp_model in inference")
            return self.model.inference(data_dict, hp_out=hp_out, sample_steps=sample_steps)
