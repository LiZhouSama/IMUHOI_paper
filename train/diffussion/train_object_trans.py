...
用法:
    ...
    python train/diffussion/train_object_trans.py --cfg config.yaml --vc_ckpt path/to/vc.pt --hp_ckpt path/to/hp.pt
    
    # 联合训练所有模块
    python train/diffussion/train_object_trans.py --cfg config.yaml --vc_ckpt path/to/vc.pt --hp_ckpt path/to/hp.pt --joint_train
"""
import os
import sys
import glob
...
from train.diffussion.loss import VelocityContactLoss, HumanPoseLoss, ObjectTransLoss
...
class Stage3JointTrainer:
    ...
    def __init__(self, cfg, vc_model, hp_model, ot_model, train_loader, test_loader=None, joint_train=False):
        ...
        if isinstance(dit_cfg, dict):
            self.ot_inference_steps = dit_cfg.get("dit_inference_steps", None)
        else:
            self.ot_inference_steps = getattr(cfg, "dit_inference_steps", None)
        if self.ot_inference_steps is not None:
            self.ot_inference_steps = int(self.ot_inference_steps)
    ...
    def forward_all(self, data_dict, batch=None, use_gt_targets=True, force_inference=False, sample_steps=None):
        """Full forward pass"""
        teacher_forcing = bool(use_gt_targets and (not force_inference) and (not self.joint_train))
        ...
        if teacher_forcing:
            ...
            hp_out = (
                self.hp_model.inference(data_dict, sample_steps=sample_steps)
                if force_inference and hasattr(self.hp_model, "inference")
                else self.hp_model(data_dict, gt_targets=batch)
            )
        ...
        hp_for_vc = hp_out if self.joint_train else ({k: (v.detach() if isinstance(v, torch.Tensor) else v) for k, v in hp_out.items()} if hp_out else None)
        vc_out = self.vc_model(
            data_dict,
            hp_out=hp_for_vc,
            gt_targets=batch if use_gt_targets else None,
            force_inference=force_inference and (not teacher_forcing),
        ...
        ot_out = self.ot_model(
            hp_for_ot['pred_hand_glb_pos'] if hp_for_ot is not None else None,
        ...
        return results, vc_out, hp_out if hp_out else {}, ot_out
    def train_epoch(self, epoch):
            ...
            for batch in train_iter:
                ...
                results, vc_out, hp_out, ot_out = self.forward_all(data_dict, batch=batch)
    ...
    def evaluate(self, epoch):
        ...
        with torch.no_grad():
            ...
            for batch in test_iter:
                ...
                results, vc_out, hp_out, ot_out = self.forward_all(
                    data_dict,
                    batch=batch,
                    use_gt_targets=False,
                    force_inference=True,
                    sample_steps=self.ot_inference_steps,
                )
