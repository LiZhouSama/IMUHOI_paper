...
class ObjectTransModule(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.gating_head = RNNWithInit(
            ...
            n_output=4,  # 左手FK、右手FK、IMU速度积分、静止保持
    ...
    def forward(
        ...
        weights_raw = F.softmax(gate_logits, dim=-1)
        # smoothing逻辑已取消：训练和推理都直接使用四路softmax权重。
        weights = weights_raw

        # 融合位置。online模式下，窗口prefix来自已知历史结果，
        # 只预测窗口内尚未知的尾部帧。
        fused_pos = torch.zeros(bs, seq_len, 3, device=device, dtype=dtype)
        imu_integrated_pos = torch.zeros_like(fused_pos)
        static_pos = torch.zeros_like(fused_pos)
        prefix_len = 0
        ...
        for t in range(prefix_len, seq_len):
            ...
            pos_static = prev_pos
            imu_integrated_pos[:, t, :] = pos_imu_integrated
            static_pos[:, t, :] = pos_static
            fused_pos[:, t, :] = (
                weights[:, t, 0:1] * l_pos_fk[:, t, :] +
                weights[:, t, 1:2] * r_pos_fk[:, t, :] +
                weights[:, t, 2:3] * pos_imu_integrated +
                weights[:, t, 3:4] * pos_static
            )
        ...
        if has_object_mask is not None:
            ...
            imu_integrated_pos = imu_integrated_pos * mask
        ...
        results = {
            ...
            "init_lhand_lb": l_lb0,
            "init_rhand_lb": r_lb0,
            "gating_contact_prob": pred_hand_contact_prob,
