...
class VelocityContactModule(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.mamba_kwargs = mamba_kwargs_from_cfg(cfg)
        ...
        for name, idx in self.body_group_indices.items():
            ...
            group_input_dims[name] = group_input_dim
        boundary_layers = int(getattr(cfg, "boundary_mamba_layers", max(1, num_layers)))
        self.boundary_encoder = PartMambaBoundary(
            group_input_dims,
            group_hidden_dim=group_hidden,
            boundary_hidden_dim=boundary_hidden,
    ...
    def _encode_body_groups(self, pose_streams):
        ...
        for name, idx in self.body_group_indices.items():
            ...
            group_inputs[name] = group_input

        return self.boundary_encoder(group_inputs)
